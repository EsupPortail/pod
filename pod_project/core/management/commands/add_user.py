...
def add_user(nom, prenom, username, email, is_staff, detail):
    pwd = ''.join([random.choice(string.digits + string.letters) for i in range(10)])
    message = u'Bonjour %(name)s,\n\n\nun compte a été créé pour vous sur %(pod)s.\n\nVoici les informations de connexion :\n- nom d\'utilisateur : %(id)s\n- mot de passe : %(mdp)s\n\nÀ bientôt sur http://%(url)s/.\n\nP.S. : pour vous connecter utilisez le bouton « Connexion » en haut à droite, puis choisissez « Authentification locale ».' % {"name": prenom, "pod": settings.TITLE_SITE, "id": username, "mdp": pwd, "url": settings.ALLOWED_HOSTS[0]}
    try:
        ...
        profile.save()
        send_mail('Votre compte sur %s' % settings.TITLE_SITE, message, settings.HELP_MAIL, [email], fail_silently=False)
