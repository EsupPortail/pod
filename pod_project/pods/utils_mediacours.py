...
def process_mediacours(media):
    ...
    if os.path.isfile("%s/%s" %(FTP_ROOT_FOLDER, media.mediapath)):
        mediaerror(media,"file found %s/%s" %(FTP_ROOT_FOLDER, media.mediapath))
...
def process_mediazipfile(media):
    ...
    media_name, ext = os.path.splitext(os.path.basename("%s/%s" %(FTP_ROOT_FOLDER, media.mediapath)))
    ...
    try:
        ...
        if xmldoc.getElementsByTagName("audio"):
            video_src = xmldoc.getElementsByTagName("audio").item(0).getAttribute("src")
        if xmldoc.getElementsByTagName("video"):
            video_src = xmldoc.getElementsByTagName("video").item(0).getAttribute("src")
        ...
        if video_src:
            try:
                ...
                if len(list_node_img):
                    #SLIDES FOLDER
                    rootFolder =  Folder.objects.get(name=pod.owner,owner=pod.owner,level=0)
                    videoFolder, created = Folder.objects.get_or_create(name=pod.slug, owner=pod.owner, parent=rootFolder)
                    slideFolder, created = Folder.objects.get_or_create(name="slides", owner=pod.owner, parent=videoFolder)
                    ...
                    for item in list_node_img[start_img:]: #skip the first
                        ...
                        if timecode >= 0 :
                            ...
                            ep, c = EnrichPods.objects.get_or_create(video=pod, title="Record Slide %s" %i, start=timecode, end=timecode+1)
                            try :
                                data = zip.read(filename)
                                if len(data):
                                    #Save the slide
                                    slide_name, ext = os.path.splitext(os.path.basename(filename))
                                    mediaerror(media, ">> slide_name %s" %slide_name)
                                    ext = ext.lower()
                                    upc_file, created = Image.objects.get_or_create(folder=slideFolder, name=slugify(pod.title+"_"+slide_name)+ext)
                                    upc_file.file.save(slugify(pod.title+"_"+slide_name)+ext, ContentFile(data), save=True)
                    ...
                    if pod.duration and pod.duration!=0 and current_enrich != None and current_enrich.start < pod.duration:
                        ...
                        mediaerror(media, "last_enrich title : %s" %last_enrich.title)
...
def save_video(media, video_data, video_src):
    ...
    ext = ext.lower()
    pod.video.save("record_"+slugify(media.title)+ext, ContentFile(video_data), save=False )
    pod.title = media.title #os.path.basename(vid.original_video.name) #on recupere le nom du fichier sur le serveur
    pod.to_encode = True 
    pod.save()
    return pod
