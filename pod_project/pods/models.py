...
from taggit.managers import TaggableManager, _TaggableManager, TaggableRel #django-taggit
...
class Channel(models.Model):
    ...
    headband = FilerImageField(null=True, blank=True, verbose_name=_('Headband'))
    color = models.CharField(_('Background color'), max_length=10, blank=True, null=True)
    
    style = models.TextField(_('Extra style'), null=True, blank=True)
    
    owner = models.ForeignKey(User, related_name='owner_channels', verbose_name=_('Owner'))
    ...
    visible = models.BooleanField(verbose_name=_('Visible'), 
        help_text=_(u'If checked, the channel appear in a list of available channels on the platform'), 
        default=False)
...
class Theme(models.Model):
    ...
    headband = FilerImageField(null=True, blank=True, verbose_name=_('Headband'))
    channel = models.ForeignKey('Channel', related_name='themes', verbose_name=_('Channel'))
...
class _MyTaggableManager(_TaggableManager):
    ...
    def __init__(self, through, model, instance, prefetch_cache_name):
        ...
        self._db = None
        self.force_lowercase = getattr(settings, 'TAGGIT_FORCE_LOWERCASE', True)
...
class Pod(Video):
    ...
    tags = MyTaggableManager(
        help_text=_(u'Separate tags with spaces, enclose the tags consist of several words in quotation marks.'), 
        verbose_name=_('Tags'), blank=True)
        
    type = models.ForeignKey(Type, verbose_name=_('Type'))
    discipline = models.ManyToManyField(Discipline, blank=True, verbose_name=_('Disciplines'))
    channel = models.ManyToManyField(Channel, blank=True, null=True, verbose_name=_('Channels'))
    theme = models.ManyToManyField(Theme, blank=True, null=True, verbose_name=_('Themes'))
    ...
    is_draft=models.BooleanField(verbose_name=_('Draft'), help_text=_(u'If you check this box, the video will be visible and accessible only by you'), default=True)
    is_restricted=models.BooleanField(verbose_name=_(u'Restricted access'), help_text=_(u'The video is accessible only by those who can authenticate to the site.'), default=False)
    password = models.CharField(_('password'),help_text=_(u'The video is available with the specified password.'), max_length=50, blank=True, null=True)
    ...
    def get_all_encoding_height(self):
        all_encoding_type = self.encodingpods_set.values_list('encodingType__output_height', flat=True).distinct()
        return all_encoding_type
    
    def get_encoding_240(self):
        encoding_240 = self.encodingpods_set.filter(encodingType__output_height=240)
        return encoding_240
        
    def get_MP4_240_URL(self):
        encoding_240 = EncodingPods.objects.get(video=self, encodingType__output_height=240, encodingFormat="video/mp4")
        return encoding_240.encodingFile.url
        
    def get_mediatype(self):
        #print "get_mediatype : %s - %s" %(self.id, self.encodingpods_set.values_list("encodingType__mediatype", flat=True).distinct())
...
class EncodingPods(models.Model):
    ...
    encodingType = models.ForeignKey(EncodingType, verbose_name=_('encodingType'))
    encodingFile = models.FileField(_('encodingFile'), null=True, upload_to=get_storage_path, blank=True, max_length=255)
    FORMAT_CHOICES = (
        ("video/mp4", 'video/mp4'),
    ...
    encodingFormat = models.CharField(_('Format'), max_length=12, choices=FORMAT_CHOICES, default="video/mp4")
...
class ContributorPods(models.Model):
    ...
    role = models.CharField(_(u'role'), max_length=200, null=True, blank=True, choices=ROLE_CHOICES, default=_("authors") )
...
class TrackPods(models.Model):
    ...
    kind = models.CharField(_('Kind'), max_length=10, choices=KIND_CHOICES, default="subtitles")
    ...
    src = FilerFileField(null=True, blank=True, verbose_name=_("Video track file"))
...
class EnrichPods(models.Model):
    ...
    stop_video = models.BooleanField(_('Stop video'), default=False, help_text=_('The video will pause when displaying this enrichment'))
    start = models.PositiveIntegerField(_('Start'), default=0, help_text=_('Start displaying enrichment in second'))
    end = models.PositiveIntegerField(_('Stop'), default=0, help_text=_('Stop displaying enrichment in second'))
    ...
    type = models.CharField(_('Type'), max_length=10, choices=CHAPTER_CHOICES, null=True, blank=True)
    
    image = FilerImageField(null=True, blank=True, verbose_name="Image", related_name="chapter_image")
    richtext = RichTextField(_('richtext'), config_name='complete', blank=True)
    weblink = models.URLField(_(u'Web link'), max_length=200, null=True, blank=True )
    document = FilerFileField(null=True, blank=True, verbose_name="Document", help_text=_('Integrate an document (PDF, text, html)'))
    embed = models.TextField(_('Embed'),max_length=300, null=True, blank=True, help_text=_('Integrate an external source'))
...
class ChapterPods(models.Model):
    ...
    time = models.PositiveIntegerField(_('Start time'), default=0, help_text=_('Start time in second of the chapter'))
...
class Building(models.Model):
    ...
    image = FilerImageField(null=True, blank=True, verbose_name="Image", related_name="building_image")
...
class Recorder(models.Model):
    name = models.CharField(_('name'),max_length=200, unique=True)
    image = FilerImageField(null=True, blank=True, verbose_name="Image", related_name="recorder_image")
    ...
    is_restricted=models.BooleanField(verbose_name=_(u'Restricted access'), help_text=_(u'Live is accessible only by those who can authenticate on the website.'), default=False)
