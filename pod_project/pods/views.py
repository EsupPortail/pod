...
from pods.forms import ChannelForm, ThemeForm, PodForm, ContributorPodsForm, DocPodsForm, TrackPodsForm, ChapterPodsForm, VideoPasswordForm, NotesForm, MediacoursesForm, EnrichPodsForm
...
def owner_channels_list(request):
    ...
    return render_to_response("channels/my_channels.html",
                          {"channels":channels, "video_count": VIDEOS.filter(channel__in=list(channels_list)).count()},
                          context_instance=RequestContext(request))
...
def channel(request, slug_c, slug_t=None):
    ...
    order_by = request.COOKIES.get('orderby') if request.COOKIES.get('orderby') else "order_by_-date_added"
    videos_list = videos_list.order_by("%s"%replace(order_by, "order_by_",""))
    ...
    return render_to_response("channels/channel.html",
                          {"channel":channel, "theme":theme, "videos":videos},
                          context_instance=RequestContext(request))
...
def channel_edit(request, slug_c):
    ...
    if not request.user.is_superuser or channel.owner != request.user:
        #return HttpResponseForbidden("<h1>Vous n'avez pas accès à cette ressource</h1>")
        messages.add_message(request, messages.ERROR, _(u'You cannot edit this channel'))
        raise PermissionDenied
        
    ThemeInlineFormSet = inlineformset_factory(Channel, Theme, form=ThemeForm, extra=0)
    ...
    return render_to_response("channels/channel_edit.html",
                          {'form':channel_form,'formset':formset, "referer":referer},
                          context_instance=RequestContext(request))
...
def types(request):
    ...
    return render_to_response("types/types.html",
                          {"types":types, "video_count": VIDEOS.filter(type__in=list(types_list)).count()},
                          context_instance=RequestContext(request))

def owners(request):
    owners_list = User.objects.filter(pod__in=VIDEOS).order_by('last_name').distinct() #User.objects.all()
    owners_filter = request.GET.get('owners_filter') if request.GET.get('owners_filter') else None
    if owners_filter:
        owners_list = owners_list.filter(last_name__iregex=r'^%s+' %owners_filter)
    #per_page = request.GET.get('per_page') if request.GET.get('per_page') and request.GET.get('per_page').isdigit() else DEFAULT_PER_PAGE
    per_page = request.COOKIES.get('perpage') if request.COOKIES.get('perpage') and request.COOKIES.get('perpage').isdigit() else DEFAULT_PER_PAGE
    ...
    return render_to_response("owners/owners.html",
                          {"owners":owners, "video_count": VIDEOS.filter(owner__in=list(owners_list)).count()},
                          context_instance=RequestContext(request))
...
def disciplines(request):
    ...
    return render_to_response("disciplines/disciplines.html",
                          {"disciplines":disciplines, "video_count": VIDEOS.filter(discipline__in=list(disciplines_list)).count()},
                          context_instance=RequestContext(request))
...
def favorites_videos_list(request):
    videos_list = Pod.objects.filter(id__in=request.user.favorites_set.all().values_list('video', flat=True))
...
def videos(request):
    ...
    return render_to_response("videos/videos.html",
                          {"videos":videos, "types":type, "owners":list_owner, "disciplines":discipline, "tags_slug":tag},
                          context_instance=RequestContext(request))
    ...
    if video.is_draft:
        if not request.user.is_authenticated():
            ...
            if request.user == video.owner or request.user.is_superuser:
                ...
                messages.add_message(request, messages.ERROR, _(u'You cannot watch this video'))
                raise PermissionDenied
    ...
    if video.password and not (request.user == video.owner or request.user.is_superuser):
        ...
        if not request.POST:
            return render_to_response(
                'videos/video.html',
                {'video': video, 'form': form, 'channel': channel, 'theme': theme, 'share_url': share_url},
                context_instance=RequestContext(request)
            )
        else:
...
def video_add_favorite(request, slug):
    ...
    if request.POST :
        ...
        favorite, create = Favorites.objects.get_or_create(user=request.user, video=video)
        if not create:
            favorite.delete()
...
def video_edit(request, slug=None):
    ...
    if request.POST:
        if video:
            video_form = PodForm(request, request.POST, request.FILES, instance=video)
        else:
    ...
    return render_to_response("videos/video_edit.html",
                          {'form':video_form, "referer":referer, "video_ext_accept":video_ext_accept},
                          context_instance=RequestContext(request))
...
def video_completion(request, slug):
    video = get_object_or_404(Pod, slug=slug)
    if request.user != video.owner and not request.user.is_superuser:
        messages.add_message(request, messages.ERROR, _(u'You cannot complete this video'))
        raise PermissionDenied
    
    ContributorInlineFormSet = inlineformset_factory(Pod, ContributorPods, form=ContributorPodsForm, extra=0, can_delete=True)
    TrackInlineFormSet = inlineformset_factory(Pod, TrackPods, form=TrackPodsForm, extra=0, can_delete=True)
    DocInlineFormSet = inlineformset_factory(Pod, DocPods, form=DocPodsForm, extra=0, can_delete=True)
    
    if request.method == "POST":
        if request.user.is_staff:
            contributorformset = ContributorInlineFormSet(request.POST, instance=video, prefix='contributor_form')
            trackformset = TrackInlineFormSet(request.POST, instance=video, prefix='track_form')
            docformset = DocInlineFormSet(request.POST, instance=video, prefix='doc_form')
            ...
            if contributorformset.is_valid() and trackformset.is_valid() and docformset.is_valid():
                ...
                docformset.save()
                #MAJ...
                contributorformset = ContributorInlineFormSet(instance=video, prefix='contributor_form')
                ...
                if request.POST.get("action3"):
                  return HttpResponseRedirect(reverse('pods.views.video', args=(video.slug,)))
                #else:
                #  return HttpResponseRedirect(reverse('pods.views.video_completion', args=(video.slug,)))
            else :
                messages.add_message(request, messages.ERROR, _(u'Error in the form'))
        else:
            contributorformset = ContributorInlineFormSet(request.POST, instance=video, prefix='contributor_form')
    ...
    if request.user.is_staff:
        return render_to_response("videos/video_completion_formset.html",
                          {'contributorformset':contributorformset, 'trackformset':trackformset, 'docformset':docformset },
                          context_instance=RequestContext(request))
    else:
        return render_to_response("videos/video_completion_formset.html",
...
def video_chapter(request, slug):
    video = get_object_or_404(Pod, slug=slug)
    if request.user != video.owner and not request.user.is_superuser:
        messages.add_message(request, messages.ERROR, _(u'You cannot chapter this video'))
        raise PermissionDenied
    
    ChapterInlineFormSet = inlineformset_factory(Pod, ChapterPods, form=ChapterPodsForm, extra=0, can_delete=True)
    
    if request.method == "POST":
        chapterformset = ChapterInlineFormSet(request.POST, instance=video, prefix='chapter_form')
            
        if chapterformset.is_valid():
            chapterformset.save()

            #MAJ...
            chapterformset = ChapterInlineFormSet(instance=video, prefix='chapter_form')
            #...
            messages.add_message(request, messages.INFO, _(u'The changes have been saved'))
...
def video_enrich(request, slug):
    ...
    EnrichInlineFormSet = inlineformset_factory(Pod, EnrichPods, form=EnrichPodsForm, extra=0, can_delete=True)
    
    if request.method == "POST":
        enrichformset = EnrichInlineFormSet(request.POST, instance=video, prefix='enrich_form')
            
        if enrichformset.is_valid():
            enrichformset.save()

            #MAJ...
            enrichformset = EnrichInlineFormSet(instance=video, prefix='enrich_form')
            #...
            messages.add_message(request, messages.INFO, _(u'The changes have been saved'))
...
def video_chapter_old(request, slug):
    ...
    if video.chapterpods_set.all() or request.is_ajax():
        ...
        ChapterInlineFormSet = inlineformset_factory(Pod, ChapterPods, form=ChapterPodsForm, extra=1, can_delete=True)
    ...
    if request.method == "POST":
        ...
        if chapterformset.is_valid():
            ...
            if request.is_ajax():
                ...
                """
                print "SAVE"
                response = render_to_string("videos/chapterformset.html", { 'chapterformset': chapterformset })
...
def video_delete(request, slug):
    video = get_object_or_404(Pod, slug=slug)
    if request.user != video.owner and not request.user.is_superuser:
        messages.add_message(request, messages.ERROR, _(u'You can\'t delete this video'))
        raise PermissionDenied
    """
    ...
    if request.method == "POST":
        video.delete()
        messages.add_message(request, messages.INFO, _(u'The video has been deleted'))
...
def get_video_encoding(request, slug, csrftoken, size, type, ext):
    csrf = request.COOKIES.get('csrftoken') if request.COOKIES.get('csrftoken') else None
    #print csrf
    #print csrftoken
    video = get_object_or_404(Pod, slug=slug)
        
    if video.is_restricted and not request.user.is_authenticated():
        raise PermissionDenied
            
    encodingpods = EncodingPods.objects.get(encodingFormat="%s/%s"%(type,ext), video=video, encodingType__output_height=size)
...
def mediacourses(request):    
    ...
    if not mediapath and not request.user.is_superuser:
        messages.add_message(request, messages.ERROR, _(u'Mediapath should be indicated'))
        raise PermissionDenied
    
    form = MediacoursesForm(request, initial={'mediapath':mediapath})
    
    if request.method == 'POST': # If the form has been submitted...
        form = MediacoursesForm(request, request.POST) # A form bound to the POST data
        if form.is_valid(): # All validation rules pass
            ...
            med.save()
            message = _('Your publication is saved. Adding it to your videos will be in a few minutes.')
...
def liveState(request): #affichage des directs
    if request.GET.get("recordingPlace") and request.GET.get("status"):
        recorder = get_object_or_404(Recorder, adress_ip = request.GET.get("recordingPlace").replace("_", "."))
